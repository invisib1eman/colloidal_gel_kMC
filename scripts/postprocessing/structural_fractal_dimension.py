...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument('--bins', type=int, required=True,
                       help='Number of bins for structure factor calculation')
    parser.add_argument('--UnitLength', type=float, required=True,
                       help='Unit length in A')
...
def save_structure_factors(u: mda.Universe, salt: float, step_size: float, n_particles: int, box_length: float, total_time: float, tau_crawl: float, freeroll: bool, bins: int , UnitLength: float) -> None:
    ...
    for j in range(0, 10):
        ...
        # Unit length is 80 A=8 nm, so k_unit is in 1/80 A^-1
        k_unit = sfDirect.bin_centers / UnitLength  
...
def main():
    ...
    save_structure_factors(u, args.salt, args.step_size, args.n_particles, args.box_length, args.total_time, args.tau_crawl, args.freeroll, args.bins, args.UnitLength)
